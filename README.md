...
## 🤖 默认推荐：Agent 内置浏览器（Browser-only）模式
...
### 2) 在课程中心页启动 runner（runner 会自动开课并注入）

在课程中心页（My Courses）执行（通过 Agent 的浏览器 evaluate）：
...
**Q: 可以同时开多门课吗？**
A: 本 Skill 面向 Agent 使用时，默认采用**单页面串行**（同一时间只学习一门课程），以降低不稳定因素并便于对话式编排。若你是纯命令行用户并自行承担稳定性风险，可自行扩展多标签方案。
